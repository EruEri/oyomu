...
let existing_term =
  Arg.(
    value
      & flag
      & info ["e"; "exist"]
      ~doc:"Indicate that the comic should already exist"
  )
...
let cmd_man = 
  [
    ...
    `P "Add comics to the collection"; 
  ]
...
let run cmd_read =
  ...
  let () = match start_index with
    | n when n < 0 -> failwith "negative volume"
    | _ -> ()
  in
  ...
  let content = match content with
    | Some c -> c
    | None when existing -> failwith @@ "No manga " ^ comic ^ "Exist"
    | None -> 
      let () = match Util.FileSys.create_folder ~on_error:(Libyomu.Error.Create_folder path) path with
        | Ok _ -> ()
        | Error e -> raise @@ Libyomu.Error.(yomu_error @@ Init_Error e)
      in
      Array.init 0 (fun _ -> String.empty) 
  in 
  let () = List.iter (fun (index, archive) -> 
    Libyomu.Collection.add ~comic_name:comic ~comic_dir:path ~comics:content index archive
  ) files in
  ()
